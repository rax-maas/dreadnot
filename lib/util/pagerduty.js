...
function PagerDuty(options, log) {
  ...
  this.pagerduty_auth = sprintf('Token token=%s', options.apiKey);
}
...
PagerDuty.prototype.override = function(schedule, user, startTime, endTime, callback) {
  ...
  reqOpts = {
    method: 'POST',
    uri: sprintf('%sapi/v1/schedules/%s/overrides', this.url, scheduleId),
    headers: {
      'Content-Type': 'application/json',
      'Authorization': this.pagerduty_auth
    },
    body: JSON.stringify({
      override: {
