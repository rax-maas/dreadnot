...
exports.spawn = function(cmd, spawnOpts, callback) {
  ...
  function closingEvent() {
    ...
    if (stdoutClosed && stderrClosed && typeof exitCode !== 'undefined') {
      log.info('Command finished', {'cmd': cmdStr, 'code': exitCode, 'stdout': resultString, 'stderr': errString});
    } else if (timedOut) {
      log.info('Command failed %s due to timeout', cmdStr);
      if (!err) {
        err = new Error(sprintf('Failed command %s: Timedout', cmdStr));
      }
    } else {
      log.info('Command failed', {'cmd': cmdStr, 'code': exitCode || null, 'stdout': resultString || null, 'stderr': errString || null});
    }
