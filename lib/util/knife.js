...
function queryAndRun(baton, args, query, cmd, msg, options, callback) {
  ...
  exports.search('node', query, options, function(err, results) {
    ...
    var eachFunc = async.forEach;
    if (options && options.parallelLimit) {
      eachFunc = function(arr, iterator, callback) {
        var parallelLimit = options.parallelLimit;
        if (typeof parallelLimit === 'function') {
          parallelLimit = parallelLimit(arr.length);
        }
        return async.forEachLimit(arr, parallelLimit, iterator, callback);
      };
    }

    eachFunc(results.rows, execute, function(_, errors) {
