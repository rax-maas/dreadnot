...
function knife(args, options) {
  return function(callback) {
    var env = misc.copyEnv(),
        knife_path_yum = '/usr/local/bin/knife',
        knife_path_apt = '/usr/bin/knife',
        knife_path;
    options = options || {};

    // Support knife on either yum or apt systems
    if (fs.existsSync(knife_path_apt)) {
      knife_path = knife_path_apt;
    } else if (fs.existsSync(knife_path_yum)) {
      knife_path = knife_path_yum;
    }
