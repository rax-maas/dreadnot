...
AuthDB.prototype.validate = function(username, password, callback) {
  ...
  } else if (this.users[username].indexOf('$2y$') === 0 || this.users[username].indexOf('$2a$') === 0 || this.users[username].indexOf('$2b$') === 0 ) {
    callback(null, this._validateBcrypt(password, this.users[username]));
  } else {
    callback(null, false);
  }
};
...
function PassThruAuthDb() {}

PassThruAuthDb.prototype.validate = function(xForwardedUser, password, callback) {
  if (xForwardedUser) {
    callback(null, true);
  } else {
    callback(null, false);
...
AuthDB.prototype._validateBcrypt = function(password, hash) {
  var all = hash.substring(hash.indexOf('$'));
  return bcrypt.compareSync(password, all);
};
...
function createProxyPassAuth() {
  return new PassThruAuthDb();
}

exports.loadDBFromFile = loadDBFromFile;

exports.createProxyPassAuth = createProxyPassAuth;
