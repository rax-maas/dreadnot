...
WebHandlers.prototype.getWarning = function(req, res, next) {
  this._render(res, 'warning.jade', {});
};


WebHandlers.prototype.saveWarning = function(req, res, next) {
  var self = this,
      text = req.body.action === 'save' ? req.body.warning_text : '';

  this._dreadnot.setWarning(text, function(err) {
    if (err) {
      res.emsg = err.message;
    }
    self.getWarning(req, res, next);
  });
};
