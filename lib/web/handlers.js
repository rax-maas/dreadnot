...
WebHandlers.prototype.renderCallback = function(res, next, template) {
  ...
  return function(err, data) {
    if (err) {
      ...
      res.render(template, {
        user: res.req.session.username,
        title: self._dreadnot.getName(),
        helpers: viewHelpers,
        data: data
      });
    }
...
WebHandlers.prototype.getLogin = function(req, res, next) {
  ...
  this.renderCallback(res, next, 'login.jade')(null, {next: next});
};
...
WebHandlers.prototype.attemptLogin = function(req, res, next) {
  ...
  this._authdb.validate(username, password, function(err, valid) {
    if (valid) {
      ...
      self.renderCallback(res, next, 'login.jade')(null, {emsg: 'Invalid Username or Password', next: next});
    }
  });
};
...
WebHandlers.prototype.deploy = function(req, res, next) {
  ...
  async.waterfall([
    ...
    function(stack, callback) {
      stack.run('deploy', regionName, to, req.session.username, callback);
    }
  ],
