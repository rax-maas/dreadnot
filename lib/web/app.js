...
function Auth(login_required) {
  this.authed = function(req, res, next) {
    var dest = '/login';
...
exports.registerPrivate = function(baton, app, auth) {
  app.get('/', auth.authed, baton.webHandlers.getStacks.bind(baton.webHandlers));
  app.get('/logout', auth.authed, baton.webHandlers.logout.bind(baton.webHandlers));
  app.get('/stacks/:stack/regions/:region', auth.authed, baton.webHandlers.getDeployments.bind(baton.webHandlers));
  app.get('/stacks/:stack/regions/:region/deployments/:deployment', auth.authed, baton.webHandlers.getDeployment.bind(baton.webHandlers));
  app.post('/stacks/:stack/regions/:region/deploy', auth.authed, baton.webHandlers.deploy.bind(baton.webHandlers));
};
...
exports.run = function(port, dreadnot) {
  ...
  var auth = new Auth(dreadnot.config.login_required);
