...
function buildAuthMiddleware(config, read) {
  return function(req, res, next) {
    ...
    if(req.isAuthenticated()) {
      req.remoteUser = new User(true, req.user.name);
      next();
    } else if (req.session && req.session.authed === true) {
...
exports.registerWeb = function(dreadnot, authdb) {
  ...
  passport.use(new GoogleStrategy({
      clientID: dreadnot.config.googleClientId,
      clientSecret: dreadnot.config.googleClientSecret,
      callbackURL: dreadnot.config.serverUrl + "auth/google/callback"
    },
